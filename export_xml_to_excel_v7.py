#!/usr/bin/env python3
"""
python export_xml_to_excel_v7.py
Adds "Item 2 Name" column to Clash Report.
Fixs 
🧩 Fix: Support both <clashresult> and <clashgroup>
...
✅ This will make your script compatible with both Navisworks output styles.
🧠 Why Navisworks uses two structures

<clashresult> → Used for individual clash results (standard clash tests).

<clashgroup> → Used when results are grouped (grouped clash reports).

Your “40R” report was generated as a grouped report, so each clash is a <clashgroup> instead of a <clashresult>.
"""
...
# --- handle both clashresult and clashgroup structures ---
    clashes = root.findall(".//clashresult")
    clash_mode = "result"
    if not clashes:
        clashes = root.findall(".//clashgroup")
    ...
    for i, clash in enumerate(clashes, start=1):
        ...
        if clash_mode == "group":
            # The 40R XML uses <clashgroup>
            # We want the <clashtest name> + <clashgroup name>
            parent_test = clash.find("../..")
            test_name = parent_test.get("name", "") if parent_test is not None else ""
            group_name = clash.get("name", "")
            clash_group = f"{group_name}, {test_name}".strip(", ")
        else:
            # Standard <clashresult> XML
